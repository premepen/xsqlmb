...
function install_luaJit(){
    cd ${DEPLOY_PATH} && \
    wget http://www.lua.org/ftp/lua-5.3.0.tar.gz && tar zvxf lua-5.3.0.tar.gz && \
    cd lua-5.3.0/ && make linux && make install
...
function make_modsecurity_with_tengine(){
    ...
    echo -e "\033[1;31m **** 开始编译安装结合 tengine+modsecurity **** \033[0m"
     if [ ! -d ${DEPLOY_PATH}/openresty-1.13.6.2 ]; then
        cd $DEPLOY_PATH && wget https://openresty.org/download/openresty-1.13.6.2.tar.gz \
